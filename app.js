...
// ============================================
// ITEMS SYNC (stub declaration - implemented later)
// ============================================
...
const dataOps = {
    ...
    async createItem(itemData) {
        ...
        // Sync to Supabase if list is shared
        await itemsSync.syncItem(item);
...
// ============================================
// ITEMS SYNC (Supabase Realtime)
// ============================================

// Extend itemsSync with full implementation (after dataOps and ui are defined)
Object.assign(itemsSync, {
    subscription: null,
    ...
    subscribe(listId) {
        ...
        this.subscription = supabaseClient
            ...
            .on(
                ...
                async (payload) => {
                    console.log('[ItemsSync] Change detected:', payload);
                ...
                for (const item of data) {
                    ...
                    if (!existingItem) {
                        await db.put(STORES.items, {
                            ...
                            inShoppingList: item.in_shopping_list !== false,
...
const navigation = {
    ...
    async openList(listId) {
        // Unsubscribe from previous items subscription
        itemsSync.unsubscribe();
        ...
        // Load items from Supabase if list is shared
        await itemsSync.loadItems(listId);

        // Subscribe to realtime changes if authenticated
        const list = appState.lists.find(l => l.id === listId);
        if (list && list.isShared && auth.isAuthenticated()) {
            itemsSync.subscribe(listId);
        }
